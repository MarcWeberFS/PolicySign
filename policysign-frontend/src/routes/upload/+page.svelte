    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="bg-white shadow-md rounded px-6 pt-6 pb-8 mb-4">
            <div class="mb-4">
                ...
                <input type="file" on:change={uploadFile} accept="application/pdf" class="shadow border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            {#if $pdfUrl}
                <div class="flex items-center justify-between">
                    <button on:click={handleNext} class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Next</button>
                </div>
            {/if}
        </div>
    </div>
...
{#if $step === 2}
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <form on:submit|preventDefault={handleSubmit} class="bg-white shadow-md rounded px-6 pt-6 pb-8 mb-4">
            <div class="mb-4">
                <label for="email" class="block text-gray-700 text-sm font-bold mb-2">E-Mail of the signee</label>
                <input type="text" bind:value={email} id="email" placeholder="E-Mail" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
            <div class="mb-4">
                <label for="title" class="block text-gray-700 text-sm font-bold mb-2">Title</label>
                <input type="text" bind:value={title} id="title" placeholder="Title" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-700 text-sm font-bold mb-2">Description</label>
                <textarea bind:value={description} id="description" placeholder="Description" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"></textarea>
            </div>
            ...
            {#if $pdfUrl}
                <div class="relative overflow-auto mb-4" style="height: {$pdfHeight};">
                    ...
                    <div class="absolute top-0 left-0 w-full h-full cursor-crosshair" on:click={onPdfClick}></div>
